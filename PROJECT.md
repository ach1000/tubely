...
`apiConfig` (`main.go`) holds `db` (`database.Client`), `jwtSecret`, `platform`, `filepathRoot`, `assetsRoot`, `s3Bucket`, `s3Region`, `s3CfDistribution`, `port`, `s3Client` (`*s3.Client`) — the env-derived fields `log.Fatal` if unset; `s3Client` is built via `config.LoadDefaultConfig(ctx, config.WithRegion(s3Region))` + `s3.NewFromConfig` (credentials come from `~/.aws/credentials` via `aws configure`, not env vars). `godotenv.Load(".env")` loads env vars locally.
...
- `videos`: `id`, `created_at`, `updated_at`, `title`, `description`, `thumbnail_url`, `video_url`, `user_id` — `thumbnail_url` is a `http://localhost:<port>/assets/<randomName>.<ext>` URL pointing at a file written to `ASSETS_ROOT` by the thumbnail upload handler; `video_url` is unwritten (video upload is a no-op stub).
