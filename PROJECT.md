...
- `videos`: `id`, `created_at`, `updated_at`, `title`, `description`, `thumbnail_url`, `video_url`, `user_id` — `thumbnail_url` is a `http://localhost:<port>/assets/<videoID>.<ext>` URL pointing at a file written to `ASSETS_ROOT` by the thumbnail upload handler; `video_url` is unwritten (video upload is a no-op stub).
...
- `handler_upload_thumbnail.go`: `handlerUploadThumbnail` is implemented — parses the multipart form (10MB max memory), reads the `thumbnail` form file, checks the authenticated user owns the video (`401` if not), determines the file extension from the `Content-Type` header (`mime.ParseMediaType`/`mime.ExtensionsByType`), and writes the bytes to `<assetsRoot>/<videoID>.<ext>` on disk (`os.Create` + `io.Copy`). Sets `thumbnail_url` to `http://localhost:<port>/assets/<videoID>.<ext>` (served by the existing `/assets/` static file server) via `cfg.db.UpdateVideo`, and responds with the updated `database.Video`. Expected to move to S3 later in the course.
